...
public:
     ...
     Mat avg_circles(int circles[][][], int b){
        int avg_x=0;
        int avg_y=0;
        int avg_r=0;
        for (int i=0;  i< b; i++ )
        ...
        Mat xyr = (Mat_<double>(3, 1) << avg_x, avg_y, avg_r);
	    return xyr;
    }

    float dist_2_pts(int x1, int y1, int x2, int y2){
        float tmp = sqrt((x2-x1)^2+(y2-y1)^2);
        return tmp;
        }


    Mat calibrate_gauge(Mat zhizhen,int gauge_number,string name, string file_type){
        ...
        Mat xyr = (Mat_<double>(8, 1) << min_angle, max_angle, min_value, max_value, units, x, y, r);
        return xyr;
        }
